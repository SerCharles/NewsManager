...
void NNews::m_OutPut()
{
	...
	delete(out_info_name); 
	delete(out_word_name); 
	delete(out_info_back); 
	delete(out_txt_back); 
}
...
NNews::NNews(NCharString* filename)
{
	...
	//m_NewsStack.m_Clear();
	delete(begin_hint);
	delete(end_hint_left);
	delete(end_tag);
	delete(space);
	delete(title_tag);
	delete(news_tag);
	delete(content_tag);
	delete(disable_tag_1); 
	delete(disable_tag_2);
	delete(disable_tag_3); 
	delete(disable_tag_4); 
	delete(news_source); 
	delete(news_source_appendix);
	delete(news_content);
	delete(news_content_appendix); 
}
