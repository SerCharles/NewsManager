...
public:
	...
	{
        ofstream file("result1.txt", std::ios::app);
    ...
    NFileLink* m_HandleWordSearchQT(NDictionary* the_dictionary, NCharString* the_line)
    {
                NCharStringLink* the_link = new NCharStringLink();
                int max_length = the_dictionary->m_MaxLength;
                int i = 0;
                ...
                //merge and search articles
                NFileLink* the_filelink = NULL;
                NChainNode<NCharString>* present = the_link->m_Head->m_Next;
                while (present != NULL)
                {
                    NFileLink* new_file = new NFileLink(present->m_Data);
                    NBinaryNode<NFileLink>* new_node = new NBinaryNode<NFileLink>(new_file);
